...
public class ShootLoop
{
    ...
    private static bool runHotKey = true;
    private static Thread childThread = null;

    public static void startRunAutoKey()
    {
        runHotKey = true;

        ThreadStart childRef = new ThreadStart(ClickKeyShoot);
        childThread = new Thread(childRef);
        childThread.Start();

    }

    public static void startRunSpeedAutoKey()
    {
        runHotKey = true;

        ThreadStart childRef = new ThreadStart(SpeedShot);
        childThread = new Thread(childRef);
        childThread.Start();

    }

    public static void stopRunAutoKey()
    {
        runHotKey = false;
        childThread.Abort();
    }
    ...
    public static void ClickKeyShoot()
    {
        while(runHotKey)
        {
            ...
            if(!CountCDTime(SKILL_SING_1)) break;//3
            FuncKeyClick(SKILL_KEY_2);
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5 
            FuncKeyClick(SKILL_KEY_3);
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5  6
            //FuncKeyClick(SKILL_KEY_1);
            if(!CountCDTime(SKILL_SING_1)) break;//3    9
            FuncKeyClick(SKILL_KEY_1);
            if (!CountCDTime(SKILL_SING_1)) break;//3   12
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5  
            FuncKeyClick(SKILL_KEY_2);
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5  15
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5 16.5
            FuncKeyClick(SKILL_KEY_3);
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5  18
            FuncKeyClick(SKILL_KEY_1);
            if(!CountCDTime(SKILL_SING_1)) break;//3   21
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5  
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5  24
            FuncKeyClick(SKILL_KEY_2);
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5 27
            FuncKeyClick(SKILL_KEY_1);
            if (!CountCDTime(SKILL_SING_1)) break;//3   30
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5
            if (!CountCDTime(UNI_CD_TIME)) break; //1.5 
            if (!CountCDTime(UNI_CD_TIME)) break; //1.5  34.5
            FuncKeyClick(SKILL_KEY_2);
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5   36
            FuncKeyClick(SKILL_KEY_1);
            if (!CountCDTime(SKILL_SING_1)) break;//3   39
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5  42
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5  43.5
            FuncKeyClick(SKILL_KEY_3);
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5  45
            FuncKeyClick(SKILL_KEY_2);
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5
            FuncKeyClick(SKILL_KEY_1);
            if (!CountCDTime(SKILL_SING_1)) break;//3   
            if (!CountCDTime(UNI_CD_TIME)) break;//1.5 51

            if (!CountCDTime(UNI_CD_TIME)) break;//1.5  
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5  54
            if(!CountCDTime(UNI_CD_TIME)) break;//1.5  
    ...
    public static void SpeedShot()
    {
        ...
        //CountCDTime(50);// 
        FuncKeyClick(SKILL_KEY_1);
        CountCDTime(SKILL_SING_1 * Speed);//1.8  
        FuncKeyClick(SKILL_KEY_2);   
        CountCDTime(UNI_CD_TIME); //1.5   3.3
        FuncKeyClick(SKILL_KEY_3);
        CountCDTime(UNI_CD_TIME);//1.5  4.8
        CountCDTime(UNI_CD_TIME); //1.5   
        CountCDTime(UNI_CD_TIME); //1.5   7.8
        FuncKeyClick(SKILL_KEY_1);
        CountCDTime(SKILL_SING_1 * Speed);  //x    9.6

        CountCDTime(2200); // 11.8
        FuncKeyClick(SKILL_KEY_2);
        CountCDTime(UNI_CD_TIME); //1.5   
        CountCDTime(UNI_CD_TIME); //1.5  14.8
        CountCDTime(1200); //            16 
        FuncKeyClick(SKILL_KEY_1);
        CountCDTime(SKILL_SING_1);//3   19
        FuncKeyClick(SKILL_KEY_3);
        CountCDTime(UNI_CD_TIME); //1.5   
        CountCDTime(UNI_CD_TIME); //1.5  22 
        FuncKeyClick(SKILL_KEY_2);      
        CountCDTime(UNI_CD_TIME); //1.5  
        CountCDTime(UNI_CD_TIME); //1.5  25
        FuncKeyClick(SKILL_KEY_1);
        CountCDTime(UNI_CD_TIME); //1.5  
        CountCDTime(UNI_CD_TIME); //1.5  28
        CountCDTime(UNI_CD_TIME); //1.5   
        CountCDTime(UNI_CD_TIME); //1.5  31
        FuncKeyClick(SKILL_KEY_3);
        CountCDTime(UNI_CD_TIME); //1.5   32.5
        FuncKeyClick(SKILL_KEY_2);
        CountCDTime(UNI_CD_TIME); //1.5  34
        FuncKeyClick(SKILL_KEY_1);
        CountCDTime(SKILL_SING_1);//3    37
        CountCDTime(UNI_CD_TIME); //1.5   
        CountCDTime(UNI_CD_TIME); //1.5   
        CountCDTime(UNI_CD_TIME); //1.5   
        CountCDTime(UNI_CD_TIME); //1.5   
        CountCDTime(UNI_CD_TIME); //1.5   
        CountCDTime(UNI_CD_TIME); //1.5   43
    ...
    public static bool CountCDTime( double cdt)
    {
